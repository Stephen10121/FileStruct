      ...
      try {
        if (!req.files) {
          ...
          if (!userDecode.where) {
            ...
            file.mv(
              `./storage/${hashed(user.usersName)}/home/${userDecode.where}/` +
                file.name
            );
          }
  ...
  jwt.verify(
    ...
    async (err, user) => {
      ...
      res.json({
        msg: "Good",
        files: files.files,
        fileSize: String(Math.floor(files.fileSize / 10000) / 100) + "Mb",
      });
    }
  );
});
  ...
  jwt.verify(
    ...
    async (err, user) => {
      ...
      const files = await getFiles(
        `./storage/${hashed(user.usersName)}/shared`
      );
      res.json({
        msg: "Good",
        files: files.files,
  ...
  jwt.verify(
    ...
    async (err, user) => {
      ...
      const fileData = await readFile(
        path.join(
          __dirname,
          `./storage/${hashed(user.usersName)}/${
            req.query["shared"] === "true" ? "shared" : "home"
          }`,
          req.query.location
        )
      );
    ...
    async (err, user) => {
      ...
      try {
        ...
        const videoPath = path.join(
          __dirname,
          `./storage/${hashed(user.usersName)}/home`,
          req.query.location
        );
...
app.post("/addFolder", async (req, res) => {
  ...
  jwt.verify(
    ...
    async (err, user) => {
      ...
      const addedFolder = await addFolder(
        req.query.location,
        req.query.name,
        user.usersName
      );
...
app.post("/deleteSharedFolder", async (req, res) => {
  ...
  jwt.verify(
    ...
    async (err, user) => {
      ...
      const deletedFolder = await deleteSharedFolder(
        req.query.location,
        user.usersName
      );
      if (deletedFolder === 200) {
  ...
  jwt.verify(
    ...
    async (err, user) => {
      ...
      const deletedFile = await deleteFile(
        req.query.location,
        user.usersName,
        req.query["shared"] === "true" ? true : false
      );
      if (deletedFile === 200) {
        const files = await getFiles(
          `./storage/${hashed(user.usersName)}/${sendLocation}`
        );
        res.json({ msg: "Good", files: files.files, fileSize: files.fileSize });
        return;
      }
...
app.post("/addFileToDrive", async (req, res) => {
  ...
  jwt.verify(
    ...
    async (err, user) => {
      ...
      const movedFolder = await addFileToDrive(
        req.query.location,
        user.usersName
      );
      if (movedFolder === 200) {
...
app.post("/shareFolder", async (req, res) => {
  ...
  jwt.verify(
    ...
    async (err, user) => {
      ...
      const sharedFolder = await shareFolder(
        req.query.location,
        user.usersName,
        req.query.user
      );
...
app.post("/shareFile", async (req, res) => {
  ...
  jwt.verify(
    ...
    async (err, user) => {
      ...
      const sharedFile = await shareFile(
        req.query.location,
        user.usersName,
        req.query.user
      );
